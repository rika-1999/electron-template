...
**Key Decisions**:
// do not update `ChannelAPI`  `setPort` is only used by test code;  
1. **Conditional `ChannelAPI` interface update**: If removing `@ts-expect-error` surfaces a type error on `setPort` access, add `setPort(port: import('@/shared/channel/impl').Port): void` to `ChannelAPI` because the method is already public on the Channel class -- the interface should reflect reality.
...
**Trade-offs Accepted**:
- **Conditional interface change**: We may modify shared type `ChannelAPI` to resolve a test-only type gap. Justified because the method IS public and the interface should reflect reality.
...
src/shared/channel/
  types.ts                          # UPDATE (expected): Add setPort to ChannelAPI -- setPort is public on Channel class but absent from ChannelAPI interface
```
...
| channelHelpers.ts | Test helper for mock Channel pairs | Remove 2 `@ts-expect-error` comments (lines 66, 70), replace 2 `as any` with `as unknown as import('@/shared/channel/impl').Port` (lines 67, 71) |
| types.ts (expected) | Channel type contract definitions | Add `setPort(port: import('@/shared/channel/impl').Port): void` to `ChannelAPI` -- method is public on Channel class but absent from interface |
...
**Integration impact**: Zero impact from deletions (no imports from deleted files). Zero impact from vi.stubEnv removal (vitest.config.mts already sets PROCESS_TYPE). channelHelpers changes are internal -- 4 consumer files use returned Channel objects without calling setPort.
...
**Context**: channelHelpers.ts uses `@ts-expect-error` to call `Channel.setPort()`, a public method not on the `ChannelAPI` interface.

**Options**:
1. Add `setPort` to `ChannelAPI` interface (makes type reflect reality)
2. Use targeted type assertion in test helper only (avoids interface change)
3. Keep `@ts-expect-error` with improved comment

**Decision**: Remove `@ts-expect-error` first. If type error surfaces, add `setPort(port: import('@/shared/channel/impl').Port): void` to `ChannelAPI` interface at `src/shared/channel/types.ts`.

**Consequences**:
- `ChannelAPI` accurately documents all public methods on Channel
- `setPort` visible to consumers of `getAllChannels(): Map<string, ChannelAPI>` -- acceptable since already public
- Renderer's `window.__app_channel__` remains unaffected (preload code curates what to expose)
...
- Verify: `npx vitest run`. If type error on `setPort` access, add `setPort` to `ChannelAPI` interface at `src/shared/channel/types.ts`
...
**Example:**
```
Read task file at .specs/tasks/draft/improve-test-practices.feature.md and implement only Step 2: Remove Redundant vi.stubEnv Calls. Do not implement any other steps. Follow the step's success criteria exactly.
```
...
```
   ...
   | Test Files     |  | vi.stubEnv     |  | + conditional ChannelAPI       |
   | [haiku]        |  | Calls          |  | interface update               |
   |                |  | [opus]         |  | [opus]                         |
   +-------+--------+  +-------+--------+  +---------------+----------------+
           |                    |                           |
           |    PARALLEL        |   PARALLEL                |
...
| Surrounding Code Preservation | 0.25 | No unrelated code was modified or accidentally deleted during the vi.stubEnv removal |
...
| No Stray Side Effects | 0.05 | No leftover comments referencing removed stubs, no orphaned imports or variables introduced by the removal |
...
### Step 3: Resolve @ts-expect-error in channelHelpers.ts (with Conditional ChannelAPI Update)
...
**Goal**: Remove 2 `@ts-expect-error` directives and replace 2 `as any` casts with proper type assertions in the test helper. If removing `@ts-expect-error` surfaces a type error on `setPort` access (because `ChannelAPI` interface at `src/shared/channel/types.ts` does not include `setPort` even though the `Channel` class exposes it publicly at `src/shared/channel/index.ts:75`), add `setPort` to the `ChannelAPI` interface.

#### Expected Output

- Modified: `src/__tests__/infrastructure/helpers/channelHelpers.ts` -- 2 `@ts-expect-error` comments removed, 2 `as any` replaced with `as unknown as import('@/shared/channel/impl').Port`
- Conditional: `src/shared/channel/types.ts` -- `setPort(port: import('@/shared/channel/impl').Port): void` added to `ChannelAPI` interface (only if type error surfaces after removing `@ts-expect-error`)

#### Success Criteria

- [ ] `grep "@ts-expect-error" src/__tests__/infrastructure/helpers/channelHelpers.ts` returns no output
- [ ] No `as any` in channelHelpers.ts (replaced with `as unknown as import('@/shared/channel/impl').Port`)
...
**Artifacts:** `src/__tests__/infrastructure/helpers/channelHelpers.ts`, `src/shared/channel/types.ts` (conditional)
**Threshold:** 4.0/5.0
...
| Criterion | Weight | Description |
|-----------|--------|-------------|
| Type Assertion Correctness | 0.25 | `as any` is replaced with `as unknown as import('@/shared/channel/impl').Port` -- the double assertion pattern is correctly applied and targets the right import path |
| @ts-expect-error Elimination | 0.20 | Both `@ts-expect-error` directives are fully removed with no remaining suppressions in channelHelpers.ts |
| Interface Accuracy (if modified) | 0.25 | If ChannelAPI was updated, `setPort` signature matches the actual Channel class implementation exactly (parameter type, return type) and is placed correctly in the interface |
| Consumer Impact Neutrality | 0.15 | No consumer of channelHelpers.ts or ChannelAPI is broken -- the 4 consumer test files continue to work without modification |
| No `any` Introduction | 0.15 | No new `any` type was introduced anywhere; the replacement uses proper type narrowing via `unknown` intermediate |

**Reference Pattern:** `src/shared/channel/types.ts` (ChannelAPI interface), `src/shared/channel/impl.ts` (Port type at line 6), `src/shared/channel/index.ts` (Channel class setPort at line 75)
...
- [ ] IF a type error surfaces on `setPort` access: add `setPort(port: import('@/shared/channel/impl').Port): void` to `ChannelAPI` interface at `src/shared/channel/types.ts` (after line 8, before the closing brace)
- [ ] Run `npx vitest run src/__tests__/main/` to verify all consumer tests pass

#### Risks

Medium. Removing `@ts-expect-error` may surface a TypeScript error on `setPort` access. The `Channel` class at `src/shared/channel/index.ts:75` has `setPort` as a public method, but `ChannelAPI` interface does not include it. If the compiler cannot resolve the method on the typed variable, add `setPort` to `ChannelAPI`. Mitigation: the agent handles both the removal and the conditional interface update atomically.

#### Definition of Done

- [ ] No `@ts-expect-error` in channelHelpers.ts
- [ ] No `as any` in channelHelpers.ts
...
| 3 | Resolve `@ts-expect-error` + conditional ChannelAPI update | Type-safe test helper | M | opus |
| 4 | Final verification | All acceptance criteria confirmed | S | sonnet |
...
| 3 | Panel (2) | 2 | 4.0/5.0 | channelHelpers.ts, types.ts (conditional) |
| 4 | None | - | - | Binary grep/test checks |

**Total Evaluations:** 4 judge runs: 2 per-file judges (Step 2) + 2 panel judges (Step 3)
**Implementation Command:** `/implement .specs/tasks/draft/improve-test-practices.feature.md`
...
|--------------|--------|------------|------------|
| `@ts-expect-error` removal causes type error | Step 3 needs interface update | Medium | Agent adds `setPort` to `ChannelAPI` interface as part of same step |
...
| Adding `setPort` to `ChannelAPI` affects consumers | Unwanted type visibility | Low | `setPort` is already public on `Channel` class; preload code curates renderer exposure |
...
- [ ] All acceptance criteria (FR-1 through FR-6) verified
- [ ] No `vi.stubEnv` calls remain in any test file
- [ ] No `@ts-expect-error` directives remain in channelHelpers.ts
- [ ] Test count reduced by 17 tests (14 + 1 + 2) from deleted files
- [ ] All remaining tests pass with exit code 0
- [ ] TypeScript compilation succeeds with no type errors
- [ ] No `any` casts introduced without proper type narrowing
